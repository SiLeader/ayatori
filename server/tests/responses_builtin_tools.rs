...
fn write_credential_file(provider_id: &str, provider_type: LlmProviderType) -> String {
    let path = unique_path("credential-builtin");
...
#[actix_web::test]
async fn openai_builtin_tools_are_passed_through() {
    ...
    assert_eq!(body["output"][0]["action"], "search");
}

#[actix_web::test]
async fn non_openai_builtin_tools_are_rejected() {
    let mock_server = MockServer::start().await;
    let app = build_app(mock_server.uri(), LlmProviderType::Anthropic, false).await;
    let req = test::TestRequest::post()
        .uri("/v1/responses")
        .set_json(json!({
            "model": "model-1",
